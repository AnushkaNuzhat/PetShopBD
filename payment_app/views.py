...
def checkout(request):
    ...
    return render(request, 'payment/checkout.html', context={'categories': categories, "form": form, "order_items": order_items, "order_total": order_total, "saved_address": saved_address})
...
def payment(request):
    ...
    if not request.user.profile.is_fully_filled():
        messages.info(
            request, f"Please update your profile details first to order!")
    ...
    mypayment.set_customer_info(name=current_user.profile.fullname, email=current_user.email, address1=current_user.profile.address, address2=current_user.profile.address,
    ...
    mypayment.set_shipping_info(shipping_to=current_user.profile.fullname, address=saved_address.address,
